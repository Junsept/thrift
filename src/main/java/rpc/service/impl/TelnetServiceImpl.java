...
public class TelnetServiceImpl implements Iface{
	...
	public Result connect(EndPoint endPoint) throws TException {
		...
		if(check(endPoint).isStatus()){
			try {			
				...
				obj.telnet = telnet;
				obj.output = telnet.getOutputStream();
				obj.input = telnet.getInputStream();
				
		    	//写命令的流
		        PrintStream pStream = new PrintStream(obj.output);
		        Thread.sleep(500);
	        	pStream.println(endPoint.username);
	        	pStream.flush();
	        	Thread.sleep(500);
	        	pStream.println(endPoint.password);
	        	pStream.flush();           
